    ...
    makeMovie(filmName, roles) {
        ...
        if (arguments.length === 2) {
            ...
            if (firstArgIsString && secondArgIsArray) {
                //we find the films, which have such name
                let foundFilms = this.films.filter((f) => f.filmName.includes(filmName));

                //we return all film roles in one object (role, and if the actor is free/not )? 
                ...
                //the film is created (with the vacant actor spots?)
                let film = {
                    filmName,
                    filmRoles
                };
                // we add to our name the count of the found films? 
                if (foundFilms.length > 0) {
                    film.filmName += ` ${++foundFilms.length}`;
                }

                // and in the end, we push the film , to the films + 
                //return the film 
